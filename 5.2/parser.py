...
def find_fuzzer_stats(root_dir, frame):
    for dirpath, _, filenames in os.walk(root_dir):
        if "trial-" in dirpath.split('/')[-1]:
            ...
            resultat = str(resultat) + f"_{frame}"
            print(resultat)
            print()
...
if __name__ == "__main__":
    for i in range(1, 91, 4):
        records = []
        root_directory = os.getcwd()
        find_fuzzer_stats(root_directory, i)
        df = pd.DataFrame(records)
        df.to_csv(f"fuzzbench-z-{i}.csv", index=False)